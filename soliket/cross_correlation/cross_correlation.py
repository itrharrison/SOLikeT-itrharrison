...
class GalaxyGalaxyLikelihood(CrossCorrelationLikelihood):
    r"""
    Likelihood for photometric galaxy positions.
    """
    _allowable_tracers: ClassVar[list[str]] = ["galaxy_density"]
    bias_mode: str | None
    ...
    def _get_theory(self, **params_values) -> np.ndarray:
        ...
        for tracer_comb in self.sacc_data.get_tracer_combinations():
            ...
            for itracer in [0, 1]:
                ...
                tracers[itracer] = ccl.WeakLensingTracer(
                    cosmo, dndz=(z_tracer, nz_tracer), ia_bias=ia_z
                )

                if self.z_nuisance_mode is not None:
                    nz_tracer = self._get_nz(
                        z_tracer, tracers[itracer], tracer_comb[itracer], **params_values
                    )
...
class ShearGalaxyLikelihood(CrossCorrelationLikelihood):
    r"""
    Likelihood for cross-correlations of galaxy weak lensing shear and CMB lensing data.
    """

    _allowable_tracers: ClassVar[list[str]] = ["galaxy_density", "galaxy_shear"]
    ...
    def _get_theory(self, **params_values) -> np.ndarray:
        ...
        for tracer_comb in self.sacc_data.get_tracer_combinations():
            if self.sacc_data.tracers[tracer_comb[0]].quantity == "galaxy_density":
                tracers_name[tracer_comb[0]] = tracer_comb[tracer_comb[0]]

                z_tracer = self.sacc_data.tracers[tracer_comb[tracer_comb[0]]].z
                nz_tracer = self.sacc_data.tracers[tracer_comb[tracer_comb[0]]].nz

                if self.bias_mode is None:
                        bias_z = None

                elif self.bias_mode == "linear":
                    bias = params_values[f"{tracers_name[tracer_comb[0]]}_b"]
                    bias_tracer = np.ones_like(z_tracer) * bias
                ...
                if self.mag_mode is None:
                    ...
                    mag_bias = params_values[f"{tracers_name[tracer_comb[0]]}_cmag"]
                    mag_bias_tracer = np.ones_like(z_tracer) * cmag
            ...
            if self.sacc_data.tracers[tracer_comb[1]].quantity == "galaxy_density":
                tracers_name[tracer_comb[1]] = tracer_comb[tracer_comb[1]]

                z_tracer = self.sacc_data.tracers[tracer_comb[tracer_comb[1]]].z
                nz_tracer = self.sacc_data.tracers[tracer_comb[tracer_comb[1]]].nz

                if self.bias_mode is None:
                        bias_z = None

                elif self.bias_mode == "linear":
                    bias = params_values[f"{tracers_name[tracer_comb[1]]}_b"]
                    bias_tracer = np.ones_like(z_tracer) * bias
                ...
                if self.mag_mode is None:
                    ...
                    mag_bias = params_values[f"{tracers_name[tracer_comb[1]]}_cmag"]
                    mag_bias_tracer = np.ones_like(z_tracer) * cmag
